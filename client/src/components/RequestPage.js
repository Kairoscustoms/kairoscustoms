...
function RequestPage() {
  const [formData, setFormData] = useState({ name: "", email: "", message: "", size: "Small" });
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepend the hoodie size info to the message.
    form.append("message", `Hoodie Size: ${formData.size}\n${formData.message}`);
    ...
    <div className="request-container">
      ...
      <form onSubmit={handleSubmit} className="request-form">
        ...
        <textarea
          ...
          required
        />
        {/* New select field for hoodie size */}
        <select
          name="size"
          value={formData.size}
          onChange={handleChange}
      ...
      {showPricing && (
        <div className="pricing-details">
          ...
          <p>(Includes: simple artwork on front or back, chest logo, small details, etc.)</p>
          ...
          <ul>
            ...
            <li>Detailed or large artwork (e.g., people or intricate illustrations): additional charge applies</li>
            <li>Specific blank over $40: (cost minus $40)</li>
          </ul>
        </div>
      )}
      <p className="request-info">
        We will review your request within 3-5 business days and email you. Shipping is additional.
      </p>
    </div>
  );
}
